...
INC= keras.applications.inception_v3.InceptionV3(input_shape=(299,299, 3), include_top=False, weights="imagenet")
...
model=tf.keras.models.Model(INC.input,x)
opt = tf.keras.optimizers.Adam(learning_rate=0.0001)
model.compile(optimizer=opt, loss = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True), metrics=["acc"])

mc = ModelCheckpoint("InceptionV3.h5", monitor='val_acc', mode='max', save_best_only=True)
...
a=model.evaluate(test_x,test_y,batch_size=32)
print(a)
